...
struct JSONDummies {
    ...
    static let posts: [JSONEntity.PostItem] = {
        try! JSONDecoder().decode([JSONEntity.PostItem].self, from: JSONDummies.postsStr.data(using: .utf8)!)
    }()
    
    static let users: [JSONEntity.User] = {
        try! JSONDecoder().decode([JSONEntity.User].self, from: JSONDummies.userStr.data(using: .utf8)!)
    }()
    
    static let comments: [JSONEntity.Comment] = {
        try! JSONDecoder().decode([JSONEntity.Comment].self, from: JSONDummies.commentsStr.data(using: .utf8)!)
    }()
